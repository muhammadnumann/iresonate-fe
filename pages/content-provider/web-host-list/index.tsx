...
// schema
import { WEB_HOST_LIST } from 'src/graphql/Queries/queries'
...
> = (props) => {
  ...
  const [getWebHostList, { data: PaginatedUserListType, loading }] =
    useLazyQuery<{ webHostList: PaginatedWebHostType }>(
      WEB_HOST_LIST,
      {
  ...
  const columns = [
    {
      ...
      render: (record: ContentWriterDetail) => `${record?.name}`,
    },
    {
    ...
    <>
      <SliderComponent>
            ...
            <WSButton
              ...
              onClick={() => {
                setModal({
                  state: true,
                  data: '',
                  typeName: formLabelName.addWebHost,
                })
              }}
            >
              {formLabelName.add}
        ...
        <WSTable
          columns={columns}
          dataSource={
            PaginatedUserListType && PaginatedUserListType['vendorList'] &&
            PaginatedUserListType['vendorList']?.nodes
          }
          loading={loading}
          pagination={{
            pageSize,
            ...
            total:
              PaginatedUserListType && PaginatedUserListType['vendorList'] &&
              PaginatedUserListType['vendorList']?.totalCount,
          }}
          bordered
          showSizeChanger={
            PaginatedUserListType && PaginatedUserListType['vendorList'] && PaginatedUserListType['vendorList']?.totalCount >= 10
              ? true
              : false
          }
...
export default withAuth(WebHostList, UserRoleType.ContentWriter)
